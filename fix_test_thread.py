...
def replace_test(match):
    ...
    # Still inside monkeypatch/mock, let the thread run
    with mock.patch("mkv_episode_matcher.backend.routers.rip.execute_gemini_fallback") as mock_exec:
        mock_exec.side_effect = lambda store, ids, *a: [ids[0]]
        time.sleep(0.1)
        
        # In the real code, if execute_gemini_fallback returns success, it just leaves it in gemini_analysis_running
        # unless it is completely finished, in which case the worker will pick it up (but there is no worker here)
        assert store.get(media_id).review_code == "gemini_analysis_running" or store.get(media_id).state != "review_required"
'''

code = re.sub(
    r'def test_manual_endpoints_queue_transition_lock\(.*?(?=\n\n|\Z)',
    replace_test,
    code,
    flags=re.DOTALL
)
