...
const JellyfinCleanupPanel = () => {
  ...
  const executeDelete = async (plan: CleanupPreview, candidateKeys?: string[]) => {
    ...
    const warning = mode === 'all_staging' ? 'This includes files without Jellyfin backups and may require reripping.' : 'Only staging files with verified Jellyfin counterparts will be removed.';
    ...
    try {
      ...
      setMessage(`Deleted ${payload.deleted_file_count} staging file(s). Jellyfin files were not changed.`);
      setPreview(null);
